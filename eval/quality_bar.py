...
Chỉ đọc kết quả có sẵn: ``eval/results/run_*.json`` (golden set), ``eval/results/ingest_*.json`` (nạp nguồn) và
``codebase/logs/decisions.jsonl`` (độ trễ theo ``trace_id``). Không gọi LLM.
...
def _fabricated(row: dict[str, Any]) -> bool:
    ...
    if row["got"] != "FOUND":
        return False
    expected_items = {ITEM_OF[s] for s in case["expected_source_ids"] + case.get("if_found_sources", [])}
    return any(s not in ITEM_OF or (expected_items and ITEM_OF[s] not in expected_items) for s in row["got_sources"])


def golden_metrics(run: dict[str, Any], latency: dict[str, int]) -> dict[str, Any]:
    """Tính Q1–Q5, Q8 cho một lượt golden set."""
    rows = run["rows"]
    groups: dict[str, list[bool]] = {}
    for r in rows:
        groups.setdefault(r["group"], []).append(r["pass"])
    conflict_rows = [r for r in rows if GOLDEN[r["id"]]["accept"] == ["CONFLICT"]]
    scope_rows = [r for r in rows if r["group"] == "hard_3_scope"]
    found_rows = [r for r in rows if GOLDEN[r["id"]]["accept"] == ["FOUND"]]
    ...
    return {
        ...
        "Q2": sum(_fabricated(r) for r in rows),
